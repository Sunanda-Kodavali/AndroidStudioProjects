...
public class MainActivity extends AppCompatActivity {
    ...
    private FakeStoreService fsService;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fsService = new FakeStoreService(new OkHttpService());
        fsService.getCategoriesAsync(result -> {
    ...
    private void updateRecyclerView(int categoryPosition) {
        if (categoryPosition == 0) {
            fsService.getProductsAsync(result -> {
                setAdapter(result);
            });
        } else {
            String category = categories.get(categoryPosition);
            fsService.getProductsByCategoryAsync(category, result -> {
                setAdapter(result);
            });
        }
    }
